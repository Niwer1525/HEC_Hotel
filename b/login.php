<?php 
	session_start();
	// Etape 4: si la session de l'utilisateur existe déjà, il n'oblige pas de s'authentifier et il renvoie directement à list_prof
	...
	if (isset($_POST['sub_btn'])){	
		...
		if(($username == "admin") AND ($password == "admin")){
			// Etape 3 : Sauvegarder le nom de l'utilisateur, puis rediriger vers la page list_prof.php
			$_SESSION['username']=$username;			
			header("Location: list_prof.php");			
			exit();
		}else{ // dans le cas contraire, afficher un message d'erreur et rester sur la même page
